...
enum ServoState {OPEN, CLOSE, WAIT, EMERGENCY, CLOSED};
...
void updateDoor(){
 ...
 if (distance <= DIST_THRESHOLD && currentDoorState != WAIT && currentDoorState != EMERGENCY){
  ...
  if ((currentDoorState != EMERGENCY || currentDoorState != CLOSED || currentDoorState != WAIT) && !door.attached()){
    door.attach(PIN_SERVO);
  }
...
void open(){
  static unsigned long laEMERGENCYenTime = 0;
  if (currentTime - laEMERGENCYenTime >= interval){
    laEMERGENCYenTime = currentTime;
    if (angle < 170){
      ++angle;
    }else {
      angle = 170;
...
bool onClickEmergency(){
  static bool state = true;
  if (state) {
    currentDoorState = EMERGENCY;
    door.detach();
    state = false; 
  }else{
    currentDoorState = CLOSE;
